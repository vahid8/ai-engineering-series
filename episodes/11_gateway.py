"""Episode 11 — RAG part 2: grounding + citations (the G — generation).

The gateway is UNCHANGED again. RAG part 2 needs exactly two endpoints, and the
gateway has had both for episodes: /v1/embeddings (to retrieve, from Ep9/10) and
/v1/chat/completions (to generate, since Ep3). Nothing new server-side — all the
new work is in the client, where we hand the retrieved chunks to the LLM and make
it answer FROM your docs, with citations. (Kept here so the episode clones-and-runs.)
