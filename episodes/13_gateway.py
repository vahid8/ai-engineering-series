...
One small gateway change this episode: we raise the rate limit. Evaluation fires
a LOT of calls (one embed + one answer + one judge per eval question), so the
demo's tiny 5-per-30s limit from Ep8 would 429 mid-run. We bump it to a realistic
60-per-minute — the same number the docs always claimed. (For real, you'd give an
admin/eval key its own higher limit rather than raise it for everyone.)

Everything else is identical; the eval harness itself lives in the client.
...
RATE_LIMIT = 60   # raised from 5 — eval makes many calls (see module docstring)
