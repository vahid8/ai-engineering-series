...
Builds on the Ep7 gateway. So far ours has been wide open: anyone who can reach
it calls any model, as often as they like, for free. To run it as a real shared
service — for teammates, customers, or your own apps — three things have to
change, and each answers one question:

  1) API keys   — WHO is calling?       identity, via the standard
                                         `Authorization: Bearer <key>` header.
  2) rate limit — how OFTEN may they?   cap requests per key per window  -> 429.
  3) budget     — how MUCH may they?    cap dollars per key (reusing the Ep5
                                         cost tracker)                    -> 402.

None of this touches the model call — it's a gate we put in FRONT of it. The keys
here are OUR gateway's own keys (not provider keys); in real life you'd store a
hash of each one in a database, not a dict in the source.
...
# --- 1) API KEYS: OUR gateway's keys -> the user each one belongs to. ----------
# These are NOT provider keys. In production you'd store a HASH of each key in a
# database alongside its user; a dict is enough to see the idea.
API_KEYS = {
    "sk-demo-alice": "alice",  # a normal user (generous budget)
    "sk-demo-carol": "carol",  # we'll use her to show the rate limit
    "sk-demo-bob": "bob",      # a tiny budget, to show the spend ceiling
}

# --- 2) RATE LIMIT: at most RATE_LIMIT requests per RATE_WINDOW seconds, per key.
RATE_LIMIT = 5
RATE_WINDOW = 30  # seconds
CALLS: dict[str, list[float]] = defaultdict(list)  # user -> recent call timestamps

# --- 3) BUDGET: a spend ceiling per user (bob's is tiny so the demo trips fast).
BUDGET_USD = {"alice": 1.00, "carol": 1.00, "bob": 0.0018}

# The running bill per user (same shape as Ep5/Ep7, now keyed by USER, not model).
...
def enforce_limits(user: str) -> None:
    ...
    # rate limit — forget calls older than the window, then count what's left.
    now = time.time()
    ...
    recent.append(now)
    # budget — has this user already spent their ceiling?
...
async def chat_completions(req: ChatRequest, user: str = Depends(authenticate)):
    # Identity already passed (Depends -> 401 if the key is bad). Now the gates:
