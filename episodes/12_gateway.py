"""Episode 12 — RAG part 3: better retrieval (chunk overlap + reranking).

The gateway is UNCHANGED. Both upgrades this episode live in the client: chunk
overlap is pure string work, and reranking is just a second use of the
/v1/chat/completions endpoint we've had since Ep3 — we ask the LLM to reorder the
candidate passages by relevance. So the server needs nothing new; a production
gateway might add a dedicated /v1/rerank endpoint, but the pattern is the same.
(Kept here so the episode clones-and-runs on its own.)
