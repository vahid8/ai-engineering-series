...
The gateway is UNCHANGED from Episode 9 — it already exposes /v1/embeddings,
which is all RAG needs on the server side. Everything new this episode lives in
the client: we chunk documents, embed every chunk, and retrieve the most
relevant ones for a question. (Kept here so the episode clones-and-runs on its own.)
